...
Before you add an image build, a registry push, a secret mount, or a
new CI include, find what already exists. Match it. Do not invent a
second practice.

The agent must understand the live setup. That includes image
config, CI includes, secret *names*, secret store type, and where
those names are consumed. Understanding is required. Guessing is a
`HOLD`.

Do not refuse discovery because secrets exist. Open the docs. Read
the config. Use names, paths, and store type. Never reprint a secret
**value** in chat, in the report, or in a committed file.

### Where truth lives

Search in this order. Stop when the fact is proven. If a source
cannot be opened, say so and ask for the URL. Do not skip a linked
doc.

1. This repository on **this branch** and on `main`. They can
   disagree. Say so.
2. The work item: acceptance criteria, description, notes, linked
   items, named example projects.
3. The host around that work item (GitLab or equivalent): wiki,
   README, CI includes, sibling projects the story names, other
   `.gitlab-ci.yml` files it points at.
4. Linked tracker or wiki pages (Jira, Confluence, or whatever this
   project actually uses) **only when the story or wiki links them**.
5. The user, only for a blank the documents left empty.

Another agent session is not a source of truth. Live docs beat
memory.

### Discovery card

Fill this before the first image or secret edit.

```
DISCOVERY
- Docs opened (repo, work item, wiki, named example, Jira, Confluence):
- Docs that would not open:
...
- Example pipeline / include the story named:
- What I will reuse:
